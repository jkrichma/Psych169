...
public class IOIO_thread_rover_tank extends IOIO_thread
{
    ...
    float dataIRCenter, dataIRRight, dataIRLeft;


    public static boolean returnBool( boolean permission){
        return permission;
    }
    ...
    public void loop() throws ConnectionLostException
    {
        ioio_.beginBatch();
            ...
            try {
                ...
                dataIRRight = irRight.read();
                dataIRLeft = irLeft.read();

               if(ma.returnBool() == false && ma.returnONorOff()== true) {
                   if (dataIRCenter > 0.3 || dataIRRight > 0.3) {
    ...
    // using this move function
    public synchronized void move (float leftSpeed /*value between 0 and 1, 0 means it doesnt move */, float rightSpeed, boolean leftForward, boolean rightForward) {
